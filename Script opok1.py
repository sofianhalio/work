...
def log(value, status='NUR HIDAYAT', color='[P1]'):
...
def log_replace(value, status='NUR HIDAYAT', color='[G1]'):
    ...
    def start(self):
        try:
            ...
            if len(frontend_domains) == 0:
                ...
                return
            self.log('GRETONGER NEVER GIVE UP {} Noob {}'.format(self.inject_host, self.inject_port))
            while True:
                socket_client, _ = socket_server.accept()
                socket_client.recv(4096)
                domain_fronting(socket_client, frontend_domains).start()
        except Exception as exception:
            self.log('FREE INTERNET INDONESIA {} port {}'.format(self.inject_host, self.inject_port), color='[R1]')
...
class domain_fronting(threading.Thread):
    ...
    def run(self):
        try:
            ...
            self.log('NUR SMART !!! {} port {}'.format(self.proxy_host, self.proxy_port))
            ...
            self.log('FREE INTERNET INDONESIA !!! [R2]({} port {})'.format(self.proxy_host, self.proxy_port), color='[R1]')
        except OSError:
...
def main():
    print(colors('\n'.join([
        ...
        '[R1]DIAM BUKAN BERARTI KALAH,[CC]',
        
        '[Y1] SCRIPT EKOR 51'
        
        '[R1] FREE INTERNET INDONESIA'
    ])))
